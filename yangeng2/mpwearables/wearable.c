...
Wearable** get_wearables(const char* file_name, int* wearable_count, int** sample_times, int* sample_count, int* latest_launch) {
    (*latest_launch) = 0;
    (*wearable_count) = 0;
    ...
    while ((read = getline(&line, &len, fp)) != (size_t) -1) {
        if (strncmp(line, "BEGIN", 5) == 0) {
            ...
            (*wearable_count) ++;
            wearables = realloc(wearables, sizeof(Wearable*) * (*wearable_count + 1));
